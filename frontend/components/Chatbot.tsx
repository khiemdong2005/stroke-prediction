...
import { chatWithAssistant, ChatHistoryItem } from "../services/gemini";
...
const Chatbot: React.FC = () => {
  ...
  const handleSend = async () => {
    ...
    const newHistory = [...chatHistory, userHistoryItem];
    setChatHistory(newHistory);
    setIsTyping(true);

    try {
      const response = await chatWithAssistant(userMsg, newHistory);
      ...
      setChatHistory((prev) => [...prev, aiHistoryItem]);
    } catch (err) {
      const errorItem: ChatHistoryItem = {
        role: "model",
        parts: [
          {
            text: "I'm sorry, I'm having trouble connecting right now. Please check your API key.",
          },
        ],
      };
          ...
          <div className="flex-grow overflow-y-auto p-4 space-y-4">
            {chatHistory.length === 0 && (
              <div className="text-center py-10 text-slate-400">
                <p>
                  Hello! Ask me anything about stroke risk factors or how our
                  platform works.
                </p>
              </div>
            )}
